...
class SeCo(torch.nn.Module):
    ...
    def __init__(self, base_encoder, emb_dim=128, num_negatives=16384, emb_spaces=3, *args, **kwargs):
        super().__init__()

        self.emb_dim = emb_dim
        self.emb_spaces = emb_spaces
        self.num_negatives = num_negatives
        self.encoder_momentum = 0.999
        self.softmax_temperature = 0.07
        # create the encoders
        # template_model = getattr(torchvision.models, base_encoder)
        template_model = base_encoder
    ...
    def forward(self, img_q, img_k):
        ...
        for i in range(self.emb_spaces):
            ...
            if i > 0:  # embedding space 0 is invariant to all augmentations
                z_neg = torch.cat([z_neg, *[z_k[j].T for j in range(self.emb_spaces) if j != i]], dim=1)
    ...
    #     optimizer = optim.SGD(params, self.learning_rate,
